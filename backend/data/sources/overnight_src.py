...
DEFAULT_STOCKS = [
    ...
    "AMD", "NFLX", "CRM", "ORCL", "ADBE", "INTC", "PYPL",
    "SQ", "SHOP", "UBER", "ABNB", "COIN", "PLTR", "SOFI",
    "MRNA", "RIVN", "LCID", "SNAP", "GME", "AMC",
]
...
DEFAULT_CRYPTO = [
    ...
    "INJUSDT", "FETUSDT", "RENDERUSDT", "DOTUSDT", "SHIBUSDT",
]

_TIMEOUT = 15
_SEC_HEADERS = {
    "User-Agent": f"QuantPulse Scanner ({settings.sec_edgar_email or 'contact@quantpulse.dev'})"
}
...
def _get(url: str, params: dict | None = None,
         headers: dict | None = None, retries: int = 3) -> dict | list | None:
...
def _compute_crypto_indicators(klines: list[dict]) -> dict:
    ...
    if quote_vols:
        indicators["avg_daily_quote_volume"] = round(
            sum(quote_vols[-20:]) / min(len(quote_vols), 20), 0
        )
...
def polygon_snapshot_all() -> dict[str, Any]:
    ...
    if not settings.polygon_api_key:
        return {}
    data = _get(f"{_POLYGON}/v2/snapshot/locale/us/market/stocks/tickers",
...
def polygon_news(symbol: str, limit: int = 3) -> list[dict]:
    ...
    return [{"title": n.get("title", ""),
             "published": n.get("published_utc", "")} for n in results]
...
def fred_series(series_id: str, limit: int = 10) -> list[dict]:
    if not settings.fred_api_key:
        return []
    data = _get(f"{_FRED}/series/observations", params={
        "series_id": series_id,
...
def sec_insider_filings(symbol: str) -> list[dict]:
    ...
    if not cik:
        return []
    filings = _get(f"https://data.sec.gov/submissions/CIK{cik}.json",
                   headers=_SEC_HEADERS)
    if not filings:
        return []
    ...
    for i, form in enumerate(forms[:80]):
        if form in ("4", "4/A") and i < len(dates) and dates[i] >= cutoff:
            insider.append({"form": form, "date": dates[i],
...
def finra_dark_pool(symbol: str) -> list:
    data = _post(
        ...
        payload={
            "fields": ["totalWeeklyShareQuantity", "totalWeeklyTradeCount",
                       "lastUpdateDate"],
            "compareFilters": [{
...
def binance_klines(symbol: str, interval: str = "1d",
                   limit: int = 30) -> list[dict]:
    data = _get(f"{_BINANCE}/klines",
                params={"symbol": symbol, "interval": interval, "limit": limit})
    if not data:
        return []
    return [{
        "open_time": k[0], "open": k[1], "high": k[2], "low": k[3],
        "close": k[4], "volume": k[5], "quote_volume": k[7], "trades": k[8],
    } for k in data]


def binance_24h_ticker(symbol: str) -> dict:
    return _get(f"{_BINANCE}/ticker/24hr", params={"symbol": symbol}) or {}


def binance_order_book(symbol: str, limit: int = 10) -> dict:
    return _get(f"{_BINANCE}/depth", params={"symbol": symbol, "limit": limit}) or {}
...
def binance_top_movers() -> list[str]:
    ...
    usdt_pairs.sort(key=lambda t: abs(float(t.get("priceChangePercent", 0))),
                    reverse=True)
    return [t["symbol"] for t in usdt_pairs[:10]
...
def discover_stock_universe(base_symbols: list[str]) -> list[str]:
    ...
    try:
        ...
        logger.info("Dynamic stock universe: %d base + %d movers = %d total",
                     len(base_symbols),
...
def discover_crypto_universe(base_pairs: list[str]) -> list[str]:
    ...
    try:
        ...
        logger.info("Dynamic crypto universe: %d base + %d movers = %d total",
                     len(base_pairs), len(movers), len(universe))
    except Exception as e:
...
def _fetch_single_stock(sym: str, snapshots: dict) -> tuple[str, dict | None]:
    ...
    try:
        ...
        if day_data.get("c") and day_data.get("o"):
            indicators["last_hour_change_pct"] = round(
                snap.get("todaysChangePerc", 0), 2
            )
...
def assemble_stock_data(symbols: list[str]) -> dict[str, dict]:
    ...
    logger.info("Overnight scanner: fetching stock data for %d tickers (parallel, %d workers)",
                len(symbols), _WORKERS)
    ...
    with ThreadPoolExecutor(max_workers=_WORKERS) as pool:
        futures = {
            pool.submit(_fetch_single_stock, sym, snapshots): sym
            for sym in symbols
        }
    ...
    logger.info("Stock pre-filter: %d/%d tickers passed → sent to Claude",
...
def assemble_crypto_data(symbols: list[str]) -> dict[str, Any]:
    ...
    logger.info("Crypto pre-filter: %d/%d pairs passed → sent to Claude",
                len(pairs), len(symbols))
